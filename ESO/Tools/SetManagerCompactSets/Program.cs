...
namespace CompactSets
{
    ...
    class Program
    {
        ...
        static readonly Regex statsRegEx = new Regex(@"Adds\s+(?<value>\d+)\s+(?<name>.+)$");
        ...
        static void Main(string[] args)
        {
            ...
            string filename = @"C:\Users\Votan.Defiant\Data\Documents\Visual Studio 2012\Projects\CompactSets\SetManager_100016.lua";
            ...
            if (sets.Count > 0)
            {
                ...
                foreach (var entry in sets)
                {
                    ...
                    {
                        var m = statsRegEx.Match(bonus[i]);
                        if (m.Success)
                        {
                            ...
                            stats[name] = value + Int32.Parse(m.Groups["value"].Value) * 2 / (i + 2) * 4 / bonus.Count;
                        }
                    }
                ...
                foreach (var entry in sets)
                {
                    ...
                    lua.Append("qualityM = ").Append(resources['M']).Append(", ");
                    lua.Append("qualityH = ").Append(resources['H']).Append(", ");
                    lua.Append("qualityS = ").Append(resources['S']).Append(", ");
