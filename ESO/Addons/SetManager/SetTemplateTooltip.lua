...
--SetManagerTooltip = WINDOW_MANAGER:CreateControlFromVirtual("SetManagerTooltip", ItemTooltipTopLevel, "SetManagerTooltipBase")
--SetManagerTooltip:SetDrawTier(0)

WINDOW_MANAGER:CreateControlFromVirtual("SetManagerTooltip", ItemTooltipTopLevel, "ZO_ItemIconTooltip")
local SetManagerTooltip = SetManagerTooltip
--local SetManagerTooltip = SetManagerTooltip
--local SetManagerTooltipContainer = SetManagerTooltipContainer

local statValuePairPool = ZO_ControlPool:New("ZO_TooltipStatValuePair", SetManagerTooltip, "SetManagerStatValuePair")
statValuePairPool:SetCustomFactoryBehavior(
	function(self)
...
do
	local orgClearLines = SetManagerTooltip.ClearLines
	function SetManagerTooltip:ClearLines()
...
local lines = {}
function SetManagerTooltip:SetTemplateItemLink(itemLink, setTemplate, equipped)
...
function SetManagerTooltip:SetSetLink(itemLink)
	self:ClearLines()
...
do
	local function onMouseEnter(rowControl)
		if rowControl.itemLink then
			InitializeTooltip(SetManagerTooltip, rowControl, TOPRIGHT, 0, -104, TOPLEFT)
			SetManagerTooltip:SetTemplateItemLink(rowControl.itemLink, rowControl:GetParent().data, true)
		end
	end
	local function onMouseExit(rowControl)
		ClearTooltip(SetManagerTooltip)
	end

	function SetManagerTooltip:SetTemplateLink(setTemplate)
		local iconTexture = "esoui/art/crafting/smithing_tabicon_armorset_up.dds"
		ZO_ItemIconTooltip_OnAddGameData(self, TOOLTIP_GAME_DATA_ITEM_ICON, iconTexture)
		self:AddVerticalPadding(24)

		local control = SetManagerTooltipContainer
		...
		SetManagerTooltip.lastLink = setTemplate
	end
end

function addon:SetPopupTooltipSetTemplate(setTemplate)
	if not SetManagerTooltip:IsHidden() and SetManagerTooltip.lastLink == setTemplate then
		self:HidePopupTooltip()
		return false
	end

	SetManagerTooltip:SetHidden(false)
	SetManagerTooltip:ClearLines()
	SetManagerTooltip:SetTemplateLink(setTemplate)

	return true
end

function addon:HidePopupTooltip()
	SetManagerTooltip:SetHidden(true)
	SetManagerTooltip.lastLink = nil
end