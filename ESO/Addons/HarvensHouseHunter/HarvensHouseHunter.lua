...
local function ShowPinMenu(pin, button)
	...
	AddMenuItem(
		"Delete",
		function()
			HarvensHouseHunter:DeletePin(pTag)
		end
	)
...
function HarvensHouseHunter:SetupMapPins()
	...
	ZO_MapPin.PIN_CLICK_HANDLERS[1][self.pinTypeId] = {
		{
			name = function(pin)
				return tostring(pin.harvensHouseHunterCustomText)
			end,
			show = function(...)
				return true
			end,
...
function HarvensHouseHunter:Initialize(eventType, addonName)
	...
	EVENT_MANAGER:UnregisterForEvent("HarvensHouseHunter", EVENT_ADD_ON_LOADED)
